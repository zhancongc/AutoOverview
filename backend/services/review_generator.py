...
【对比分析示例】
❌ 错误写法（简单列举）：
  张三(2019)认为媒体关注度能抑制盈余管理。李四(2020)发现媒体关注度会增加盈余管理。王五(2021)指出媒体关注度的影响取决于公司治理水平。
...
【写作深度要求】
- 分析要深入，不能简单罗列
- 每个主题都要形成"文献矩阵"，展示不同研究之间的对话
- 在结论部分总结主要分歧点，指出未来研究方向
- 字数：3000-5000字
...
2. 在综述的适当位置添加这些文献的引用，优先添加到能够形成对比分析的段落
3. **对比分析优先**：优先将新文献添加到能够与现有研究形成对比或补充的位置，而非简单罗列
4. 保持原文结构和内容主体不变，必要时可以微调句子以融入新的引用
5. 每添加一个引用，在相关句子末尾加上 [序号]
6. 请直接输出修改后的完整综述内容，不要解释
...
"""
        ...
        try:
            response = await self.client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "你是学术写作助手，负责在综述中补充文献引用，特别擅长通过对比分析的方式融入新的研究观点。"},
